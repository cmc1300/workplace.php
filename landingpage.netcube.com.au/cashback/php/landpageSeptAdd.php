...
function sendEmail($name,$phone,$email,$code,$message){
	...
	*/
	$email_to = 'sales@netcube.com.au,antonio.diiorio@netcube.com.au,noorul.sabha@netcube.com.au,natasha.ivanovska@netcube.com.au,eric.liang@netcube.com.au,eram@netcube.com.au,lucy@netcube.com.au,jack.zhang@netcube.com.au,boson.huang@novatel.com.au,jerry.xu@netcube.com.au';
...
else if (!empty($_REQUEST['senderName'])) {
	$name = $_REQUEST['senderName'];
	$phone = $_REQUEST['senderPhone'];
	$email = $_REQUEST['senderEmail'];
	$code = $_REQUEST['code'];
	$formTag = "BOTTOM";
	$message = "";
	if (!empty($_REQUEST['senderMessage'])) {
		$message = $_REQUEST['senderMessage'];
	}
}
if (empty($code)) {
	$code= "CASHBACK";
...
if(true){
	if (LandpageSept::checkPhoneAndEmail($phone,$email)) {
		//print "<script>alert('Email $email and/or phone number $phone already taken, please try again');</script>";
		//print "<script>history.back()</script>";
		//exit;
	}
	else if(LandpageSept::addLandpage($name,$phone,$email,$code,$formTag,$adminUserID)) {
