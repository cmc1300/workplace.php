...
class quickpay_service
{
         ...
         /*     echo "<pre>";
            ...
            case quickpay_conf::RESPONSE:
                ...
                if (is_array($args)) {
                    ...
                    if (preg_match($pattern, $args, $match)) { //extract cupReserved first
                        $cupReserved = $match[1];
                    }
    ...
    static function sign($params, $sign_method)
    {
        ...
        if (strtolower($sign_method) == "md5") {
            ...
            foreach ($params as $key => $val) {
             ...
             $skey = md5(quickpay_conf::$security_key);
    ...
    static function curl_call($url, $data = null, $is_post = true, $options = null) 
    {
        if (function_exists("curl_init")) {
          ...
          //  file_put_contents('test.txt', var_export($ret_data, true));//add by indika
