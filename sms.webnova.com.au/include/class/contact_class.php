...
/**************************contact list******************************************/
	 ...
	 * */
	 function queryContact(){
		//If Netcube, access to all contact lists. 
	 	if ($this->company_id == '28'){
			$SQL="SELECT a.*,(SELECT COUNT(b.`contact_id`) FROM zy_contact b WHERE b.`contact_list` LIKE CONCAT('%[',a.`contact_list_id`,']%')) contact_list_count FROM zy_contact_list a ORDER BY contact_list_id DESC";
	 	}else{
//	 	$sql="select zy_contact_list.* from zy_contact_list where `company_id`='$this->company_id' ORDER BY contact_list_id DESC limit ".($GLOBALS['pages']-1)*$GLOBALS['displaypg'].",".$GLOBALS['displaypg'];
	 ...
	 * */
	 function queryAdminContact(){	 	
	 	$sql="SELECT * FROM `zy_company`,zy_contact_list WHERE zy_company.`company_id` = zy_contact_list.`company_id` ORDER BY contact_list_id DESC";
...
//	  }
	...
	function CountCustomerContact(){
	 	if($this->company_id == '28'){
	 		$sql="SELECT a.`contact_list_id`,a.`contact_list_name`,(SELECT COUNT(b.`contact_id`)  FROM zy_contact b WHERE b.`contact_list` LIKE  CONCAT('%[',a.`contact_list_id`,']%')) contact_list_count FROM zy_contact_list a"; 
